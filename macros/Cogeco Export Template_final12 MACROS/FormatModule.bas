...
Private Sub collectLogEntries()
    ...
    'currentSourceVersionHistoryTableCandidate.Select
        ...
        If (currentSourceVersionHistoryHeaderCandidate.OutlineLevel = wdOutlineLevel1 Or _
            currentSourceVersionHistoryHeaderCandidate.OutlineLevel = wdOutlineLevel2) And _
            LCase(Trim(Replace(currentSourceVersionHistoryHeaderCandidate.Range.Text, Chr(13), Chr(32)))) = "history of changes" Then
    ...
    Set entries = Utils.extractEntriesFromTable(sourceVersionHistoryTable, 3, 1)
...
'        End If
        ...
        Else
            ...
            If vhHeadingCandidate.Range.Style = "Table Title Large" _
                    And LCase(Trim(Replace(vhHeadingCandidate.Range.Text, Chr(13), Chr(32)))) = "history of changes" Then
...
Private Sub fillInTemplateVersionhistoryTable()
    ...
    For Each currentHistoryEntry In entries.LogEntries
        With templateVersionHistoryTable
            .Rows.Add
            .Cell(row, 1).Range.Text = Format(currentHistoryEntry.ChangeDate, "dd-mm-yyyy") & Chr(7)
            .Cell(row, 2).Range.Text = currentHistoryEntry.Author & Chr(7)
            .Cell(row, 3).Range.Text = currentHistoryEntry.Section & Chr(7)
            .Cell(row, 4).Range.Text = currentHistoryEntry.Description & Chr(7)
        End With
        row = row + 1
...
Private Sub setColumnWidth()
    ...
    templateVersionHistoryTable.Columns.Item(2).Width = Application.InchesToPoints(1.2)
    templateVersionHistoryTable.Columns.Item(3).Width = Application.InchesToPoints(2.2)
    templateVersionHistoryTable.Columns.Item(4).Width = Application.InchesToPoints(2.45)
